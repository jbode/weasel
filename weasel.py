...
def randomChar():
  idx = int( random.random() * len( alphabet ) )
  return alphabet[ idx ]
...
def main():
  ...
  for i in xrange(int(options.size)):
    ...
    population.append( (generation, candidate, score(candidate, target)) )

  population.sort( cmpWeasel )

  print "%15s%15s%8s%s" % ("Generation", "Score", " ", "Value")
  print "%15s%15s%8s%s" % ("----------", "-----", " ", "-----")

  print "%15d%15d%8s%s" % (population[0][0], population[0][2], " ", population[0][1] )
  ...
  while population[0][2] != 0:
    ...
    for x in xrange(breeder_max,int(options.size)):
      candidate = mutate( population[x%breeder_max][1] )
      population[x] = (generation, candidate, score( candidate, target ))
    population.sort( cmpWeasel )
